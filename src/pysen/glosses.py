...
class Gloss(object):
    ...
    def similarity(self, other):
        ...
        if set([p.lower() for p in self.parts]) == set([p.lower() for p in
            other.parts]):
            return 6 if pos else 5
        if self.parts.intersection(other.parts):
            return 4 if pos else 3
        if set([p.lower() for p in self.parts]).intersection(set([p.lower() for p in
            other.parts])):
    ...
    def from_string(
        ...
        return parse_gloss(
                s, pos=pos, language=language, splitter=splitter,
                marker=marker, brackets_open=brackets_open,
                brackets_close=brackets_close)[0]
...
def parse_gloss(
    ...
    for constituent in constituents:
        if constituent.strip():
            ...
            mainpart = ''.join(m for m in mainpart if m not in marker)\
                .strip().split()
...
def to_concepticon(
    ...
    for concept in concepts:
        ...
        if gloss:
            glosses = parse_gloss(gloss, pos=pos, marker=marker,
                    splitter=splitter, language=language,
                    brackets_open=brackets_open, brackets_close=brackets_close)
            candidates = []
            if gloss in mappings[language]:
                candidates += [(gloss, Gloss.from_string(gloss,
                    language=language, pos=pos))]
            elif gloss.lower() in mappings[language]:
                candidates += [(gloss.lower(), Gloss.from_string(gloss,
                    language=language, pos=pos))]
            for g in glosses:
            ...
            for result in results:
                matches[gloss] += [[result[0], result[1], result[3],
                    result[4]]]
        else:
